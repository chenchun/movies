...
var ExperimentParticle = function(data, position) {
  ...
  this.intro = this.data.intro.replace(' ', '&nbsp;');
...
ExperimentParticle.prototype.mouseOver = function() {
  ...
  srcTimeout = setTimeout(function() {
    ...
    domImage.src = _this.data.imgUrl;
  }, 50);
  
  //domAuthor.innerHTML = this.authorName;
  domIntro.innerHTML = this.intro;
...
ExperimentParticle.prototype.update = function(time) {
  ...
  if (this.index == 3) {
	  console.log("radius="+this.radius +" maxVel="+this.maxVel+" targetRadius"+this.targetRadius+" targetMaxVel="+this.targetMaxVel);
  }
